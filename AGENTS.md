...
- Nếu cần deploy fix hành vi nút `footer-back-top` hoặc partial JS footer chung của home renderer theo cách backup-first, dùng `node .\tools\deploy_footer_back_top_fix_live.mjs` hoặc `npm run deploy:footer-backtop-live`; script backup 3 file renderer/footer, ghi live qua hosting MCP, purge cache và ghi report/CSV.
...
- Nếu muốn chạy trọn quy trình deploy + verify footer interaction bằng 1 lệnh, dùng `npm run release:footer-backtop-live`; để test wrapper không ghi live, dùng `npm run release:footer-backtop-live -- --dry-run`.
