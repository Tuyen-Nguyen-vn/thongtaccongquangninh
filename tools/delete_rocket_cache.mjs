...
await mcpPost("initialize",{protocolVersion:"2024-11-05",capabilities:{},clientInfo:{name:"delcache",version:"1"}});
...
const CACHE_ROOT="/public_html/wp-content/cache/wp-rocket/thongtaccongquangninh.com";
// list cache pages
const ls=await call("list_files",{path:CACHE_ROOT});
console.log("cache dirs:", ls.slice(0,600));
// delete gioi-thieu cache
for(const fname of ["index.html","index.html_gzip","index-https.html","index-https.html_gzip"]){
  const p=`${CACHE_ROOT}/gioi-thieu/${fname}`;
  const del=await call("delete_file",{path:p});
  console.log("delete",p,"=>",del.slice(0,100));
}
