...
// Cum "cau hoi thuong gap thong tac cong" duoc dung nhu keo dinh giua cac
// doan (khong phai tu khoa xep hang chinh - do la "thong tac cong quang
// ninh"). Giu 3 lan dau (tieu de + mo bai), rut gon cac lan sau thanh
// "cau hoi nay" / "nhom cau hoi nay".
const FULL_RE = /câu hỏi thường gặp thông tắc cống/gi;
const KEEP_FIRST_N = 3;
...
async function main() {
  ...
  raw = raw.replace(FULL_RE, (match) => {
    ...
    return match[0] === match[0].toUpperCase() ? "Câu hỏi này" : "câu hỏi này";
  });
  ...
  const backupDir = `${ROOT}/backups/destuff-3175-${stamp}`;
