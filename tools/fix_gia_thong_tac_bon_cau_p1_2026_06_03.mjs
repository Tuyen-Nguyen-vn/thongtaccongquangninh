...
const TARGET = {
  id: 2357,
  collection: "posts",
  slug: "gia-thong-tac-bon-cau-quang-ninh",
  url: "https://thongtaccongquangninh.com/gia-thong-tac-bon-cau-quang-ninh/",
};
...
const NEW_TITLE = "Giá thông tắc bồn cầu Quảng Ninh 2026: báo giá rõ từng trường hợp";
const NEW_DESC =
...
const backupDir = join(ROOT, "seo-revisions", `wp-before-p1-gia-bon-cau-${stamp}`);
const reportPath = join(ROOT, "reports", `p1-gia-bon-cau-fix-${dryRun ? "dryrun" : "apply"}-${stamp}.json`);
...
function serviceSchemaHtml() {
  const schema = {
    ...
    name: "Giá thông tắc bồn cầu Quảng Ninh",
    serviceType: "Thông tắc bồn cầu tại Quảng Ninh",
    url: TARGET.url,
    provider: {
      "@type": "LocalBusiness",
  ...
  return `<!-- wp:html -->\n<script type="application/ld+json" data-ttcqn-service-schema="gia-bon-cau-quang-ninh">${JSON.stringify(
    schema,
  )}</script>\n<!-- /wp:html -->`;
}
...
function updateContent(raw) {
  ...
  content = content.replace(
    /<h2>Bồn Cầu Tắc Có Dấu Hiệu Gì\? Khi Nào Cần Gọi Thợ Ngay\?<\/h2>/u,
    "<h2>Nguyên nhân bồn cầu tắc và dấu hiệu cần gọi thợ</h2>",
  );
  ...
  if (content.includes('data-ttcqn-service-schema="gia-bon-cau-quang-ninh"')) {
    const before = content;
    content = content.replace(
      /<!-- wp:html -->\s*<script type="application\/ld\+json" data-ttcqn-service-schema="gia-bon-cau-quang-ninh">[\s\S]*?<\/script>\s*<!-- \/wp:html -->/u,
      serviceBlock,
    );
...
async function wp(path, options = {}) {
  const response = await fetch(`${baseUrl}/wp-json${path}`, {
    ...
    headers: {
      ...
      "User-Agent": "Codex P1 Gia Bon Cau cleanup",
    },
...
async function fetchPublic() {
  ...
  url.searchParams.set("codex", `gia-bon-cau-${Date.now()}`);
  const response = await fetch(url, {
    headers: { "User-Agent": "Codex P1 Gia Bon Cau verifier" },
  });
...
async function main() {
  ...
  const report = {
    ...
    before: {
      ...
      hasServiceSchema:
        raw.includes('data-ttcqn-service-schema="gia-bon-cau-quang-ninh"') || /"@type"\s*:\s*"Service"/iu.test(raw),
    },
    update: {
      ...
      hasServiceSchema: update.content.includes('data-ttcqn-service-schema="gia-bon-cau-quang-ninh"'),
  ...
  report.ok =
    ...
    Boolean(
      report.verify?.status === 200 &&
        ...
        /giá thông tắc bồn cầu/iu.test(report.verify?.h1?.[0] || "") &&
