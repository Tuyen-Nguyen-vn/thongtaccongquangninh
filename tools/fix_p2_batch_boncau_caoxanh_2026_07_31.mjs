...
const PAGES = [
  { id: 2043, type: "posts", slug: "bon-cau-rut-cham-nguyen-nhan" },
  { id: 2039, type: "posts", slug: "thong-tac-cong-cao-xanh" },
];
...
async function main() {
  ...
  const backupDir = `${ROOT}/backups/fix-batch7-p2-${stamp}`;
  ...
  if (!DRY) {
    writeFileSync(`${ROOT}/reports/fix-batch7-p2-${stamp}.json`, JSON.stringify(results, null, 2), "utf8");
