...
const slugs=["gia-thong-tac-cong-quang-ninh-2026","hut-be-phot-khu-cong-nghiep-quang-ninh-2026"];
for(const slug of slugs){
  for(const type of ["posts","pages"]){
    const r=await wpReq(`/wp-json/wp/v2/${type}?slug=${slug}&_fields=id,slug,type`);
    if(Array.isArray(r)&&r[0]){console.log(`${slug}: ${type} id=${r[0].id}`);break;}
  }
}
