...
const phpCode = `<?php
/**
 * Plugin Name: TTCQN Hook Debug (one-shot)
 * Description: Debug filters on the_title.
 * Version: 1.0
 */
if (!defined('ABSPATH')) exit;

add_action('wp_head', function() {
    if (!is_page(296)) return;
    global $wp_filter;
    $results = [];

    $results[] = "=== the_title Filters ===";
    if (isset($wp_filter['the_title'])) {
        foreach ($wp_filter['the_title']->callbacks as $priority => $callbacks) {
            foreach ($callbacks as $idx => $cb) {
                ...
                if (is_string($func)) {
                    ...
                    $name = (is_string($func[0]) ? $func[0] : get_class($func[0])) . '::' . $func[1];
                } elseif ($func instanceof Closure) {
                    $ref = new ReflectionFunction($func);
                    $name = "Closure at " . $ref->getFileName() . ":" . $ref->getStartLine();
                } else {
                    $name = "Unknown type";
                }
                $results[] = "Priority: {$priority} - {$name}";
            }
        }
    }

    $results[] = "\\n=== get_the_title Filters ===";
    if (isset($wp_filter['get_the_title'])) {
        foreach ($wp_filter['get_the_title']->callbacks as $priority => $callbacks) {
            foreach ($callbacks as $idx => $cb) {
                $func = $cb['function'];
                $name = "";
                if (is_string($func)) {
    ...
    // Dump also the title before filters
    $results[] = "\\n=== post_title in loop ===";
    $results[] = "post_title raw: " . get_post(296)->post_title;
    $results[] = "get_the_title(296): " . get_the_title(296);

    $logFile = WP_CONTENT_DIR . '/uploads/hook-debug-result.txt';
...
async function main() {
  const zip = buildZip([["ttcqn-hook-debug/ttcqn-hook-debug.php", Buffer.from(phpCode, "utf8")]]);
  ...
  console.log("Uploading Hook Debug plugin...");
  ...
  // Trigger plugin by making a GET request to page 296
  console.log("Triggering hook debug via HTTP GET to page 296...");
  await new Promise((resolve) => {
    const req = https.request({
      hostname: SERVER_IP, port: 443, servername: WP_HOST, path: "/thong-tac-cong-ha-long/?cache-buster=" + Date.now(),
      headers: { Host: WP_HOST, "User-Agent": "Codex Hook Debug trigger" }, rejectUnauthorized: false
    }, (res) => {
      res.on("data", () => {});
      res.on("end", resolve);
    });
  ...
  // Fetch results
  console.log("Fetching hook debug results...");
  const results = await new Promise((resolve) => {
    const req = https.request({
      hostname: SERVER_IP, port: 443, servername: WP_HOST, path: "/wp-content/uploads/hook-debug-result.txt",
  ...
  console.log("\nDeactivating Hook Debug plugin...");
  await mcpPost({
    ...
    params: {
      ...
      arguments: {
        ...
        parameters: { plugin: "ttcqn-hook-debug/ttcqn-hook-debug.php" }
      }
    }
  });
  console.log("Deleting Hook Debug plugin...");
  await mcpPost({
