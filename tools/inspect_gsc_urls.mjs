...
function markdown(report) {
  ...
  for (const item of report.results) {
    if (!item.ok) {
      const error = String(item.error?.message ?? item.status ?? "").replaceAll("|", "\\|");
      lines.push(`| ${item.url} | ERROR | ${error} |  |  |  |  |`);
      continue;
    }
    lines.push(
...
async function main() {
  if (!URLS.length) {
    console.error("Usage: node tools/inspect_gsc_urls.mjs <url> [url...]");
    process.exit(2);
  }
  ...
  const reportPath = path.join(ROOT, "reports", `gsc-url-inspection-${stamp}.json`);
  const markdownPath = path.join(ROOT, "reports", `gsc-url-inspection-${stamp}.md`);
  ...
  appendFileSync(
    CSV_PATH,
    `\n${date},${time},GSC-INSPECT-TARGET-URLS-${date},gsc_inspection,inspect targeted URLs after update/indexing,${URLS.join("|")},,${report.errorCount === 0 ? "done" : "needs_review"},low,,,,,Ran GSC URL Inspection for targeted URLs,tools/inspect_gsc_urls.mjs,,Theo doi crawl lai trong GSC neu lastCrawlTime chua doi,"${evidence}",${report.errorCount === 0 ? "PASS" : "CHECK"},Codex,${date},${reportPath},${markdownPath},,,`,
    "utf8",
  );
