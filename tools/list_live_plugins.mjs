...
await mcpPost("initialize",{protocolVersion:"2024-11-05",capabilities:{},clientInfo:{name:"plist",version:"1"}});
...
const raw = await call("list_wordpress_plugins",{domain:"thongtaccongquangninh.com"});
let arr; try{arr=JSON.parse(raw);}catch{console.log(raw.slice(0,2000));process.exit(0);}
const list = Array.isArray(arr)?arr:(arr.plugins||arr.data||[]);
console.log("TOTAL plugins:", list.length);
const act=list.filter(p=>p.active||p.status==="active"||p.is_active);
console.log("ACTIVE:", act.length);
for(const p of list){
  const name=p.name||p.slug||p.plugin||p.file||JSON.stringify(p).slice(0,60);
  const active=(p.active||p.status==="active"||p.is_active)?"ON ":"off";
