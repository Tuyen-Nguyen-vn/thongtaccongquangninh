/**
 * Push thong-tac-cong-cam-pha article to WordPress
 * Usage: node tools/push_thong_tac_cong_cam_pha.mjs
 */
...
const MD_FILE = path.join(PROJECT, "content-drafts", "thong-tac-cong-cam-pha-2026.md");
const IMG_DIR = path.join(PROJECT, "Ảnh Đã Xử Lý SEO");
const CSV_PATH = path.join(PROJECT, "docs", "SEO_PROGRESS.csv");

const IMG_FILES = [
  "thong-tac-cong-cam-pha-tho-xu-ly-cong-khu-mo.webp",
  "thong-tac-cong-cam-pha-may-lo-xo-cong-bun-than.webp",
  "thong-tac-cong-cam-pha-xe-hut-bun-khu-cong-nghiep.webp",
];
...
function markdownToHtml(md) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("#### ")) { flushP(); flushL(); out.push(`<h4>${inlineMd(line.slice(5))}</h4>`); continue; }
...
async function main() {
  ...
  const slug = field(md, "Slug") || "thong-tac-cong-cam-pha";

  console.log("=== Push: thong-tac-cong-cam-pha ===");
  ...
  const csvRow = [
    `\n2026-06-16`, t, "CONTENT-TTC-CAMPHA-001", "content_publish",
    "thông tắc cống Cẩm Phả", postUrl, slug, "published", "hard",
    "", "", "", "",
    `Publish bài thong-tac-cong-cam-pha ID=${postId} kèm 3 ảnh WebP qua IP bypass`,
    "content-drafts/thong-tac-cong-cam-pha-2026.md", "",
    "Chấm Rank Math trong WP Admin",
    "Tiếp tục bài 3: nao-vet-ho-ga-quang-ninh",
