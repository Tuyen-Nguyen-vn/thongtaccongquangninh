...
await mcpPost("initialize",{protocolVersion:"2024-11-05",capabilities:{},clientInfo:{name:"rtpl",version:"1"}});
...
const t=await call("read_file",{path:"/public_html/wp-content/plugins/ttcqn-title-meta-short-2026-06-12/ttcqn-title-meta-short-2026-06-12.php"});
console.log("=== ttcqn-title-meta-short ===\n",t);
// check seo-cleanup-redirects context around line 307
const s=await call("read_file",{path:"/public_html/wp-content/plugins/ttcqn-seo-cleanup-redirects/ttcqn-seo-cleanup-redirects.php"});
const lines=s.split("\n");
const idx=lines.findIndex(l=>l.includes("Hồ sơ Môi Trường"));
console.log("\n=== seo-cleanup-redirects ctx (±10 lines around hit) ===");
console.log(lines.slice(Math.max(0,idx-10),idx+15).join("\n"));
