...
await mcpPost("initialize",{protocolVersion:"2024-11-05",capabilities:{},clientInfo:{name:"scan",version:"1"}});
...
// search plugins for remaining old brand (exclude mcp-temp orphans + .fbbak)
const r=await call("search_files",{path:"/public_html/wp-content/plugins",query:"Môi Trường Đô Thị Số 1",search_type:"content",max_results:50});
try{
  const j=JSON.parse(r);
  const items=(j.items||[]).filter(x=>!x.path.includes("mcp-temp")&&!x.path.endsWith(".fbbak"));
  console.log("ACTIVE-plugin hits (excl mcp-temp/.fbbak):", items.length);
  for(const it of items) console.log("  "+it.path+":"+it.line+"  "+(it.match||"").slice(0,90));
  const orphan=(j.items||[]).filter(x=>x.path.includes("mcp-temp")||x.path.endsWith(".fbbak"));
  console.log("\n(orphan/backup hits ignored:", orphan.length, ")");
}catch{console.log(r.slice(0,800));}
