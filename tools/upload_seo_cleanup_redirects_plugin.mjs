...
const PLUGIN_SLUG = "ttcqn-seo-cleanup-redirects";
...
const REPORT_PATH = fromRoot(`WORDPRESS_SEO_CLEANUP_REDIRECTS_UPLOAD_${REPORT_STAMP}.json`);
...
const baseUrl = (fileEnv.WP_BASE_URL || shellEnv.WP_BASE_URL || "https://thongtaccongquangninh.com").replace(/\/$/, "");
const username = fileEnv.WP_USERNAME || shellEnv.WP_USERNAME;
const password = fileEnv.WP_APP_PASSWORD || shellEnv.WP_APP_PASSWORD;
...
async function rpc(method, params, id, sessionId) {
  const headers = {
    ...
    "User-Agent": "Codex SEO cleanup redirects upload",
  };
...
let innerUpload = null;
try {
  innerUpload = JSON.parse(uploadText);
} catch {}
...
const uploadSucceeded =
  ...
  (innerUpload?.success !== false && innerUpload?.data?.success !== false);
const success = uploadSucceeded && activeText.includes(PLUGIN_SLUG);
...
const report = {
  ...
  uploadText,
  activePluginFound: activeText.includes(PLUGIN_SLUG),
};
