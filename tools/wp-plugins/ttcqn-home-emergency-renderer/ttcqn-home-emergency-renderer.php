...
const TTCQN_HOME_EMERGENCY_VERSION = '2026.06.28.7';
const TTCQN_HOME_OFFICE_HA_LONG_ADDRESS = '111 Cái Lân, Bãi Cháy, Quảng Ninh';
const TTCQN_HOME_OFFICE_HA_LONG_MAP_URL = 'https://www.google.com/maps/search/?api=1&query=20.962384198791902%2C107.05220536959965';
const TTCQN_HOME_OFFICE_HA_LONG_MAP_EMBED = 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d931.4385482896339!2d107.05220536959965!3d20.962384198791902!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3135ad947ef4a1ff%3A0xb82f0e88497fbc1c!2zSMO6dCBC4buDIFBo4buRdCAtIE3DtGkgdHLGsOG7nW5nIMSRw7QgdGjhu4sgc-G7kSAxIFF14bqjbmcgTmluaA!5e0!3m2!1svi!2s!4v1781835498222!5m2!1svi!2s';
const TTCQN_GOOGLE_SITE_VERIFICATION_TOKEN = 'FreOU2Y0MbnSTgqPF-zN-OBKkk8XQxpzA5KM_U-owHM';

function ttcqn_home_emergency_meta_description(): string
{
    return 'Hút bể phốt, thông tắc cống, bồn cầu, hố ga tại Quảng Ninh. Hoạt động 05:00-22:00 hằng ngày, có mặt 15 phút, báo giá trước, không đục phá. Gọi ngay 0963.953.533 / 0931.156.756 để xử lý.';
}

function ttcqn_home_normalize_google_map_embed_markup(string $html): string
{
    $new_src = esc_url(TTCQN_HOME_OFFICE_HA_LONG_MAP_EMBED);
    $legacy_sources = [
        'https://maps.google.com/maps?q=M%C3%B4i+Tr%C6%B0%E1%BB%9Dng+%C4%90%C3%B4+Th%E1%BB%8B+S%E1%BB%91+1+Qu%E1%BA%A3ng+Ninh,+111+C%C3%A1i+L%C3%A2n,+B%C3%A3i+Ch%C3%A1y,+H%E1%BA%A1+Long&#038;output=embed',
        'https://maps.google.com/maps?q=M%C3%B4i+Tr%C6%B0%E1%BB%9Dng+%C4%90%C3%B4+Th%E1%BB%8B+S%E1%BB%91+1+Qu%E1%BA%A3ng+Ninh,+111+C%C3%A1i+L%C3%A2n,+B%C3%A3i+Ch%C3%A1y,+H%E1%BA%A1+Long&output=embed',
        'https://www.google.com/maps?q=111%20C%C3%A1i%20L%C3%A2n%2C%20B%C3%A3i%20Ch%C3%A1y%2C%20Qu%E1%BA%A3ng%20Ninh&#038;output=embed',
        'https://www.google.com/maps?q=111%20C%C3%A1i%20L%C3%A2n%2C%20B%C3%A3i%20Ch%C3%A1y%2C%20Qu%E1%BA%A3ng%20Ninh&output=embed',
    ];

    return str_replace($legacy_sources, $new_src, $html);
}
...
// =====================================================================
// DELAY GA4 SCRIPT — dùng script_loader_tag (an toàn, không ob_start)
// Đổi script GA4 async sang defer để giảm TBT
// =====================================================================
add_filter('script_loader_tag', function (string $tag, string $handle, string $src): string {
    // Defer JS theme không quan trọng với first paint (menu, back-to-top)
    if (in_array($handle, ['generate-menu', 'generate-back-to-top', 'generate-a11y'], true)) {
        if (!str_contains($tag, ' defer') && !str_contains($tag, ' async')) {
            $tag = str_replace('<script ', '<script defer ', $tag);
        }
        return $tag;
    }
    $delay_domains = ['googletagmanager.com', 'google-analytics.com', 'doubleclick.net'];
    foreach ($delay_domains as $domain) {
        if (str_contains($src, $domain)) {
            // Thêm defer nếu chưa có (GA4 dùng async, đổi sang defer giảm TBT hơn)
...
function ttcqn_schema_local_business_node(): array
{
    ...
    return [
        ...
        'alternateName'    => ['Hút Bể Phốt Quảng Ninh', 'Thông Tắc Cống Quảng Ninh'],
        'url'              => home_url('/'),
        'logo'             => $logo_url,
        'image'            => $logo_url,
        'description'      => 'Dịch vụ hút bể phốt, thông tắc cống, thông tắc bồn cầu, nạo vét hố ga tại Quảng Ninh. Hoạt động từ 05:00 đến 22:00 hằng ngày, có mặt trong 15 phút, không đục phá, bảo hành dài hạn.',
        ...
        'geo'              => [
            '@type'     => 'GeoCoordinates',
            'latitude'  => 20.9623842,
            'longitude' => 107.0528491,
        ],
        'department'       => [
            // TP. Hạ Long — văn phòng chính, có Google Maps đã xác minh
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-ha-long'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TP. Hạ Long',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '111 Cái Lân, Bãi Cháy',
                ...
                'hasMap'       => 'https://maps.app.goo.gl/gbKEWZQjHR6yP9iW7',
                'openingHours' => 'Mo,Tu,We,Th,Fr,Sa,Su 05:00-22:00',
                'priceRange'   => '₫₫',
            ],
            // TP. Cẩm Phả — chưa có GBP, có địa chỉ + geo
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-cam-pha'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TP. Cẩm Phả',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '416 Đường Trần Phú, Phường Cẩm Trung',
            ...
            // TP. Uông Bí — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-uong-bi'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TP. Uông Bí',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '192 Đường Quang Trung, Phường Thanh Sơn',
            ...
            // TP. Móng Cái — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-mong-cai'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TP. Móng Cái',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '88 Đường Hùng Vương, Phường Trần Phú',
            ...
            // TX. Quảng Yên — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-quang-yen'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TX. Quảng Yên',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '134 Đường Lê Lợi, Phường Quảng Yên',
            ...
            // TX. Đông Triều — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-dong-trieu'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng TX. Đông Triều',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '465 Đường Nguyễn Bình, Phường Đông Triều',
                    'addressLocality' => 'Đông Triều',
                    'addressRegion'   => 'Quảng Ninh',
                    'addressCountry'  => 'VN',
                ],
                'geo'          => [
            ...
            // Huyện Tiên Yên — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-tien-yen'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng Huyện Tiên Yên',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '72 Đường Lý Thường Kiệt, Thị trấn Tiên Yên',
                    'addressLocality' => 'Tiên Yên',
                    'addressRegion'   => 'Quảng Ninh',
                    'addressCountry'  => 'VN',
                ],
                'geo'          => [
            ...
            // Huyện Vân Đồn — chưa có GBP
            [
                '@type'        => ['LocalBusiness', 'HomeAndConstructionBusiness'],
                '@id'          => home_url('/#localbusiness-van-don'),
                'name'         => 'Thông Tắc Cống Quảng Ninh - Văn Phòng Huyện Vân Đồn',
                'telephone'    => ['+84963953533', '+84931156756'],
                'address'      => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => '318 Đường Lý Anh Tông, Thị trấn Cái Rồng',
    ...
    <header class="ttcqn-header ttcqn-shared-header" role="banner" data-ttcqn-shared-header>
        <nav class="home-nav ttcqn-nav" role="navigation" aria-label="Điều hướng chính">
            <div class="nav-inner ttcqn-nav-inner">
                <a href="<?php echo esc_url($home_url); ?>" class="nav-logo seo-header-logo ttcqn-brand" aria-label="Trang chủ Thông Tắc Cống Quảng Ninh">
                    <img src="<?php echo esc_url($logo_url); ?>" alt="Thông Tắc Cống Quảng Ninh - hút bể phốt, thông tắc cống xử lý nhanh" class="ttcqn-brand-full-logo" width="356" height="94" loading="eager" fetchpriority="high" decoding="async">
                </a>
...
function ttcqn_seo_hero_current_title(): string
{
    if (is_singular()) {
        ...
        return $title !== '' ? $title : 'Hút bể phốt, thông tắc cống Quảng Ninh';
    }
    ...
    return 'Hút bể phốt, thông tắc cống Quảng Ninh';
}
...
function ttcqn_seo_hero_highlight_title(string $title, string $highlight = 'Xử lý nhanh'): string
{
    $title = trim($title);
    if ($title === '') {
        return '';
...
function ttcqn_seo_hero_callback_form(string $prefix, array $args = []): string
{
    ...
    $defaults = [
        ...
        'area' => '',
        'issue' => '',
        'headingTag' => 'h2', // FIX LỖI 2: cho phép dùng div để tránh H2 trùng lặp
    ];
    $args = array_merge($defaults, $args);
    $heading_tag = in_array($args['headingTag'], ['h2', 'h3', 'div', 'p'], true) ? $args['headingTag'] : 'h2';
    ...
    <div class="ttcqn-seo-callback-card">
        <<?php echo $heading_tag; ?> id="<?php echo esc_attr($prefix); ?>-title"><?php echo esc_html($args['title']); ?></<?php echo $heading_tag; ?>>
...
function ttcqn_seo_hero_render(array $args = []): string
{
    ...
    $mobile_form = ttcqn_seo_hero_callback_form($uid . '-mobileform', [
        ...
        'area' => $args['defaultArea'],
        'issue' => $args['defaultIssue'],
        'headingTag' => 'div', // FIX LỖI 2: tránh H2 trùng lặp trong mobile sheet
    ]);
    ...
    <section class="ttcqn-seo-hero" data-ttcqn-seo-hero aria-labelledby="<?php echo esc_attr($uid); ?>-heading">
        ...
        <div class="ttcqn-seo-hero-inner">
            ...
            <div class="ttcqn-seo-hero-visual">
                ...
                <img class="ttcqn-seo-hero-staff" src="<?php echo esc_url($args['staffImage']); ?>" srcset="<?php echo esc_url($args['staffImageSmall']); ?> 480w, <?php echo esc_url($args['staffImageMobile']); ?> 640w, <?php echo esc_url($args['staffImage']); ?> 900w" sizes="(max-width: 767px) 220px, 300px" alt="Kỹ thuật viên Thông Tắc Cống Quảng Ninh tư vấn dịch vụ" width="900" height="1350" loading="eager" fetchpriority="high" decoding="async">
...
function ttcqn_shared_footer_enqueue_assets(): void
{
    $is_home_request = function_exists('ttcqn_home_emergency_is_front_request')
        && ttcqn_home_emergency_is_front_request();

    if (!$is_home_request && !ttcqn_shared_footer_should_render()) {
        return;
    }

    $asset_path = __DIR__ . '/assets/ttcqn-shared-footer.css';
    ...
    wp_enqueue_style(
        'ttcqn-shared-footer',
        plugins_url('assets/ttcqn-shared-footer.css', __FILE__),
        [],
        (string) filemtime($asset_path)
    );
}
add_action('wp_enqueue_scripts', 'ttcqn_shared_footer_enqueue_assets', 20);
...
(function() {
<?php include __DIR__ . '/templates/shared-footer-interactions-inline.php'; ?>
})();
...
function ttcqn_seo_hero_strip_legacy_top_blocks(string $html): string
{
    ...
    $html = ttcqn_home_normalize_google_map_embed_markup($html);
    $html = ttcqn_home_delay_analytics_markup($html);
...
function ttcqn_home_delayed_analytics_script(): string
{
    return <<<'HTML'
	<script id="ttcqn-delayed-analytics">
		(function(w,d){
    ...
    d.head.appendChild(s);
		  ...
		  function startAnalytics() {
		    if (w.__ttcqnAnalyticsLoaded) return;
		    w.__ttcqnAnalyticsLoaded = true;
		    loadScript('https://www.googletagmanager.com/gtag/js?id=AW-18031795119');
		    w.gtag('js', new Date());
		    w.gtag('config', 'AW-18031795119');
		    w.gtag('config', 'G-F2BXPJYKEG');
		    loadScript('https://www.googletagmanager.com/gtm.js?id=GTM-5N59RCXC');
		  }
  if ('requestIdleCallback' in w) {
    w.addEventListener('load', function(){ w.requestIdleCallback(startAnalytics, { timeout: 3500 }); }, { once:true });
  } else {
    w.addEventListener('load', function(){ setTimeout(startAnalytics, 1800); }, { once:true });
  }
})(window,document);
</script>
HTML;
...
function ttcqn_home_normalize_meta_description_markup(string $html): string
{
    if (!ttcqn_home_emergency_is_front_request()) {
        return $html;
    }

    $description = esc_attr(ttcqn_home_emergency_meta_description());
    $replacements = [
...
function ttcqn_home_delay_analytics_markup(string $html): string
{
    if (!ttcqn_home_emergency_is_front_request()) {
        return $html;
    }

    $html = ttcqn_home_normalize_meta_description_markup($html);

    $patterns = [
        '~<!--\s*Google Tag Manager\s*-->[\s\S]*?<!--\s*End Google Tag Manager\s*-->~i',
        '~<script\b[^>]*src=["\']https://www\.googletagmanager\.com/gtag/js\?id=(?:AW-18031795119|G-F2BXPJYKEG)[^"\']*["\'][^>]*>\s*</script>\s*~i',
        '~<script\b[^>]*>\s*window\.dataLayer\s*=\s*window\.dataLayer\s*\|\|\s*\[\];\s*function\s+gtag\(\)\s*\{\s*dataLayer\.push\(arguments\);\s*\}\s*gtag\(\s*["\']js["\']\s*,\s*new\s+Date\(\)\s*\);\s*gtag\(\s*["\']config["\']\s*,\s*["\'](?:AW-18031795119|G-F2BXPJYKEG)["\']\s*\);\s*</script>\s*~is',
    ];
    ...
    if (strpos($html, 'id="ttcqn-delayed-analytics"') === false) {
        $html = (string) preg_replace('~</body>~i', ttcqn_home_delayed_analytics_script() . "\n</body>", $html, 1);
    }
...
// FIX LỖI 5+6: Schema @id fix tạm thời bỏ qua — sẽ sửa qua Rank Math UI

function ttcqn_should_noindex_current_request(): bool
{
    if (is_search()) {
        return true;
    }
    ...
    return in_array($slug, $low_value_slugs, true);
}

// Để Rank Math render một robots meta duy nhất, tránh noindex/index mâu thuẫn.
add_filter('rank_math/frontend/robots', function ($robots) {
    if (!ttcqn_should_noindex_current_request()) {
        return $robots;
    }
...
function ttcqn_detect_service_and_location($slug) {
    ...
    if (strpos($slug, 'ha-long') !== false || strpos($slug, 'bai-chay') !== false || strpos($slug, 'cao-xanh') !== false || strpos($slug, 'hong-gai') !== false || strpos($slug, 'gieng-day') !== false || strpos($slug, 'tuan-chau') !== false) {
        ...
        if (strpos($slug, 'bai-chay') !== false) {
            ...
            $specials = 'phục vụ nhanh các nhà hàng, khách sạn dọc đường Hạ Long và khu phố cổ Bãi Cháy vào giờ cao điểm để hạn chế ảnh hưởng hoạt động kinh doanh';
        ...
        if (strpos($slug, 'minh-thanh') !== false) {
            ...
            $specials = 'kiểm tra kỹ hố ga thấp, bùn cát sau mưa, cống bếp dầu mỡ và tuyến thoát nước của nhà dân, cửa hàng, nhà xưởng tại Quảng Yên';
        }
        ...
        $specials = 'hoạt động từ 05:00 đến 22:00 hằng ngày, ưu tiên không đục phá và cam kết bảo hành dài hạn từ 6 đến 24 tháng cho mọi công trình tại Quảng Ninh';
    }
...
function ttcqn_get_localized_faq($slug) {
    ...
    $a1 = "Chi phí " . mb_strtolower($service) . " tại " . $location . " phụ thuộc vị trí tắc, độ sâu đường ống, mức bùn mỡ, thời điểm thi công và việc có cần nạo vét hố ga hay không. Kỹ thuật sẽ khảo sát, nói rõ phương án và báo giá trước khi làm. Gọi 0963.953.533 / 0931.156.756 để mô tả hiện trạng và nhận tư vấn nhanh.";

    $q2 = "Thợ " . $service . " tại " . $location . " bao lâu thì có mặt?";
    $a2 = "Đội kỹ thuật phục vụ theo tuyến khu vực và có thể hỗ trợ nhanh khi khách cung cấp địa chỉ, ảnh hiện trạng, vị trí hố ga hoặc miệng thoát. Thời gian có mặt thực tế phụ thuộc khoảng cách, thời tiết và khung giờ gọi. Riêng tại " . $location . ", kỹ thuật ưu tiên " . $specials . ".";

    $q3 = "Dịch vụ " . $service . " tại " . $location . " có bảo hành không và cam kết thế nào?";
    $a3 = "Dịch vụ " . mb_strtolower($service) . " tại " . $location . " có chính sách bảo hành theo tình trạng thực tế, thường từ 6 đến 24 tháng tùy hạng mục. Cam kết chính gồm: ưu tiên không đục phá, báo giá trước khi làm và xả thử sau xử lý để hạn chế tắc lại.";
...
function ttcqn_get_localized_quick_takeaways($slug) {
    ...
    $html .= '<span>•</span> TÓM TẮT NHANH';
    $html .= '</div>';
    $html .= '<ul class="ttcqn-quick-takeaways-list">';
    $html .= '<li><strong>Dịch vụ:</strong> ' . esc_html($service) . ' tại <strong>' . esc_html($location) . '</strong>, tiếp nhận trong khung giờ 05:00 - 22:00 và ưu tiên điều phối nhanh khi khách cần xử lý gấp.</li>';
    $html .= '<li><strong>Tốc độ:</strong> Kỹ thuật điều phối theo tuyến gần nhất; thời gian có mặt phụ thuộc địa chỉ, thời tiết và khung giờ gọi.</li>';
    ...
    $html .= '<li><strong>Hotline liên hệ trực tiếp:</strong> Gọi <a class="ttcqn-quick-takeaway-link" href="tel:0963953533">0963.953.533</a> hoặc <a class="ttcqn-quick-takeaway-link" href="tel:0931156756">0931.156.756</a> để được tư vấn và báo giá trước khi làm.</li>';
    $html .= '</ul>';
    $html .= '</div>';
...
function ttcqn_inject_faq_schema_to_head() {
    ...
    echo "\n<!-- TTCQN Localized FAQ Schema for AI Overview -->\n";
    echo "<script type=\"application/ld+json\">\n";
    echo json_encode($json_ld, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
    echo "\n</script>\n<!-- End TTCQN Localized FAQ Schema -->\n";
}

// Chèn JSON-LD WebSite Schema (Site Name) vào trang chủ để hỗ trợ Google hiển thị chính xác tên trang web thân thiện
add_action('wp_head', 'ttcqn_inject_website_schema_to_homepage', 1);
function ttcqn_inject_website_schema_to_homepage() {
    if (is_admin()) {
        return;
    }

    // Xác định trang chủ bằng nhiều cách để tránh bỏ sót
    $is_home_request = false;
    if (function_exists('is_front_page') && is_front_page()) {
        $is_home_request = true;
    } elseif (function_exists('is_home') && is_home()) {
        $is_home_request = true;
    } else {
        global $post;
        if ($post && isset($post->ID) && (int)$post->ID === 23) {
            $is_home_request = true;
        }
    }
    ...
    if ($is_home_request) {
        ...
        $website_schema = ttcqn_schema_home_graph();

        echo "\n<!-- TTCQN WebSite Schema (Google Site Name Support) -->\n";
        echo "<script type=\"application/ld+json\" data-ttcqn-home-schema=\"1\">\n";
        echo json_encode($website_schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
        echo "\n</script>\n<!-- End TTCQN WebSite Schema -->\n";
    }
}

add_action('wp_head', 'ttcqn_inject_google_site_verification_meta', 0);
function ttcqn_inject_google_site_verification_meta() {
    if (is_admin()) {
        return;
    }
    ...
    echo "\n<meta name=\"google-site-verification\" content=\"" . esc_attr(TTCQN_GOOGLE_SITE_VERIFICATION_TOKEN) . "\" />\n";
}
